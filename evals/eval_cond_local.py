...
def load_numpy_as_dataloader(numpy_data, batch_size=16):
...
def calculate_fid_from_numpy(numpy_data1, numpy_data2, batch_size=16, device='cuda'):
    # GPU 또는 CPU 설정
...
fake_list = sorted(glob.glob('/home/jionkim/workspace/_eval/fake/*.nii.gz'))
real_list = sorted(glob.glob('/home/jionkim/workspace/_eval/real/*.nii.gz'))
...
# for idx in range(0, 2):
    ...
    # Calculate FID
    fake_fid = np.expand_dims(fake_fid, axis=1)
    real_fid = np.expand_dims(real_fid, axis=1)
    ...
    metrics['FID'].append(np.average(fid_value).astype(float))
...
print('[EVALUATION (AVG)] [MAE %f] [PSNR %f] [SSIM %f] [MMD %f] [FID %f] [LPIPS %f]' % (st.mean(mae_final), st.mean(psnr_final),
                                                    st.mean(ssim_final), st.mean(mmd_final), st.mean(fid_final), st.mean(lpips_final)))

# print STD
print('[EVALUATION (STD)] [MAE %f] [PSNR %f] [SSIM %f] [MMD %f] [FID %f] [LPIPS %f]' % (st.pstdev(mae_final), st.pstdev(psnr_final),
