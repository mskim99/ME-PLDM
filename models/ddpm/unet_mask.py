...
class DiffusionWrapper(nn.Module):
    ...
    def forward(self, x, cond, t, c_s, c_m, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            out = self.diffusion_model(x, cond=cond, timesteps=t, c_s=c_s, c_m=c_m, lc=192)
...
class UNetModel(nn.Module):
    ...
    def forward(self, d, cond=None, timesteps=None, c_s=None, c_m=None, context=None, y=None, lc=0):
        """
        ...
        if c_s is not None:
            c_s_xy = c_s[:, :, 0:16 * 24].view(c_s.size(0), c_s.size(1), 16, 24)
            c_s_yt = c_s[:, :, 16 * 24:(16 + 8) * 24].view(c_s.size(0), c_s.size(1), 8, 24)
            c_s_xt = c_s[:, :, (16 + 8) * 24:(16 + 8 + 8) * 24].view(c_s.size(0), c_s.size(1), 8, 24)
        ...
        if c_s is not None:
            h_xy = torch.cat([h_xy, cond, c_s_xy, c_m_xy], dim=2)
            h_yt = torch.cat([h_yt, cond, c_s_yt, c_m_yt], dim=2)
            h_xt = torch.cat([h_xt, cond, c_s_xt, c_m_xt], dim=2)
        else:
            h_xy = torch.cat([h_xy, cond, c_m_xy], dim=2)
            h_yt = torch.cat([h_yt, cond, c_m_yt], dim=2)
            h_xt = torch.cat([h_xt, cond, c_m_xt], dim=2)
