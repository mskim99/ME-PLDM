...
class ViTAutoencoder(nn.Module):
    def __init__(self,
        ...
        self.to_pixel2 = nn.ConvTranspose3d(2048, 1, kernel_size=(8, 8, 8), stride=8)
        # self.to_pixel3 = nn.ConvTranspose3d(512, 1, kernel_size=(2, 2, 2), stride=2)
    ...
    def decode_from_sample(self, h, cond=None):
        ...
        h_yt = h[:, :, latent_res * latent_res:latent_res * (latent_res + 16)].view(h.size(0), h.size(1), 16,
                                                                                    latent_res)
        h_xt = h[:, :, latent_res * (latent_res + 16):latent_res * (latent_res + 16 + 16)].view(h.size(0), h.size(1),
                                                                                                16, latent_res)
